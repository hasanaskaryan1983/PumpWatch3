...
/**
 * 🚀 Sprint 15 (فاز ۱ / Commit 11-fix): تست‌های pure موتور خروج
 *
 * قانون جدید تست‌ها (درس Commit 11):
 *  - سناریوهای مسیر TRAIL/TIGHTEN با profitR ≥ 1 باید partialClose = true
 *    بگذارند (یعنی TP1 قبلاً گرفته شده) وگرنه PARTIAL شلیک می‌کند.
 *  - سناریوهای PARTIAL باید live = closes.last() بگذارند وگرنه
 *    VOL_SPIKE (که اولویت بالاتر دارد) شلیک می‌کند.
 */
class ExitEngineTest {
    ...
    fun volatility_spike_closes_immediately() {
        val d = ExitEngine.decide(
            ctx(closes = listOf(100.0, 100.2, 100.0, 100.2, 100.0), live = 95.0)
        )
        assertEquals("CLOSE", d.action)
    ...
    // ---------- 🚀 Commit 11: PARTIAL (TP1) ----------

    @Test
    fun partial_fires_at_one_r_when_not_yet_partialed() {
        // live = closes.last() تا VOL_SPIKE شلیک نکند
        val closes = rising(22) // 100..121
        val d = ExitEngine.decide(ctx(closes = closes, live = 121.0, partialClose = false))
        assertEquals("PARTIAL", d.action)
        assertEquals("TP1", d.reason)
        assertEquals(121.0, d.exitPrice!!, 0.0001)
    }

    @Test
    fun partial_skipped_if_already_partialed() {
        val closes = rising(22)
        val d = ExitEngine.decide(ctx(closes = closes, live = 121.0, partialClose = true))
        assertEquals("TRAIL", d.action)
        assertTrue("after TP1 the runner still trails with BE locked", d.newStop!! >= 100.0)
    }

    @Test
    fun vol_spike_beats_partial() {
        // profitR = 1.2 ولی ریزش شارپ از آخرین کندل => کل ترید بسته می‌شود
    ...
    @Test
    fun stop_beats_partial() {
        val d = ExitEngine.decide(ctx(closes = rising(22), live = 89.0))
        assertEquals("CLOSE", d.action)
        assertEquals("STOP", d.reason)
    }

    // ---------- مسیر TRAIL (همه با partialClose = true: TP1 گرفته شده) ----------
    ...
    fun breakeven_locked_after_one_r() {
        val d = ExitEngine.decide(ctx(closes = rising(30), live = 129.0, partialClose = true))
        assertEquals("TRAIL", d.action)
    ...
    fun chandelier_ratchets_tighter_as_profit_grows() {
        val d1 = ExitEngine.decide(ctx(closes = rising(25), live = 124.0, target = null, partialClose = true))
        val d2 = ExitEngine.decide(ctx(closes = rising(31), live = 130.0, target = null, partialClose = true))
    ...
    fun short_side_is_mirror() {
        val d = ExitEngine.decide(
            ctx(
                side = "DUMP", entry = 100.0, stop = 110.0, target = null,
