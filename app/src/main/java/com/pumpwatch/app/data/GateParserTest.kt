...
class GateParserTest {
    ...
    fun `candle maps open index 5 and close index 2 for Gate`() {
        ...
        assertNotNull(c)
        // مقادیر مورد انتظار پس از نگاشت صحیح:
        ...
        // Gate timestamp را به ثانیه می‌فرستد؛ باید به میلی‌ثانیه تبدیل شود
    ...
    @Test
    fun `candle uses default zero for missing optional fields`() {
        // فقط timestamp و close؛ بقیه غایب
        val partial = listOf("1700000000", "105.0")
        val c = MultiExchange.candle(partial, 0, 5, 3, 4, 2, 1, false)
        // چون index 5 در آرایه نیست، open باید 0.0 بشود نه null (پس کندل valid می‌ماند)
        assertNotNull(c)
    }
}
