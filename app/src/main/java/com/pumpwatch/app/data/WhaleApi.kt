 ...
 * هر implementation مسئول:
 *  1. نرمال‌سازی نماد
 *  2. تبدیل فرمت خودش به AggTradeNormalized
 *  3. برگرداندن لیست خالی در صورت خطا (نه throw) — برای زنجیرهٔ fallback
 */
...
object BinanceProvider : WhaleProvider {
    ...
    override suspend fun fetchNormalized(symbol: String, limit: Int): List<AggTradeNormalized> = try {
...
object BybitProvider : WhaleProvider {
    ...
    override suspend fun fetchNormalized(symbol: String, limit: Int): List<AggTradeNormalized> = try {
        ...
        if (resp.retCode != 0) return@try emptyList()
...
object OkxProvider : WhaleProvider {
    ...
    override suspend fun fetchNormalized(symbol: String, limit: Int): List<AggTradeNormalized> = try {
        ...
        if (resp.code != "0") return@try emptyList()
 ...
 * چون دادهٔ عمومی زنجیره است، هیچ مسدودسازی جغرافیایی رویش اثر ندارد.
 */
object GeckoDexProvider : WhaleProvider {
    ...
    override suspend fun fetchNormalized(symbol: String, limit: Int): List<AggTradeNormalized> = try {
        ...
        val pool = pools.maxByOrNull { it.attributes?.volume?.h24 ?: 0.0 } ?: return@try emptyList()
        val net = pool.relationships?.network?.data?.id ?: return@try emptyList()
        val poolAddr = pool.id?.substringAfter('_') ?: return@try emptyList()
...
private fun sharedOkHttp(): OkHttpClient = OkHttpClient.Builder()
...
private fun normalizeSymbol(symbol: String, exchange: String): String {
    val base = symbol.uppercase(Locale.US)
        ...
        .replace("USDT", "-USDT")
        .replace("USD", "-USD")
        .let { if (!it.contains("-")) "$it-USDT" else it }
    val parts = base.split("-")
    if (parts.size != 2) return symbol.uppercase(Locale.US)
    val (coin, quote) = parts
...
/**
 * زنجیرهٔ fallback — Commit ۲ از این استفاده می‌کند.
